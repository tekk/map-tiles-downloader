...
class Menu:
	def __init__(self, stdscr: Any, title: str, choices: List[str], multi: bool = False, all_toggle: bool = False):
		self.stdscr = stdscr
		self.title = title
...
def _estimate_avg_tile_size_sync(url_builder: Any, headers: Dict[str, str], regions: Dict[str, Tuple[float, float, float, float]], min_zoom: int, max_zoom: int) -> float:
